<?php

namespace App\Http\Middleware;

use App\Models\Tenant;
use Closure;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Config;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use Symfony\Component\HttpFoundation\Response;

class SetTenantConnection
{
    public function handle(Request $request, Closure $next): Response
    {
        $slug = $this->resolveSlug($request);

        if (!$slug) {
            abort(404, 'Prefeitura não encontrada.');
        }

        $tenant = Tenant::where('slug', $slug)
            ->where('is_ativo', true)
            ->first();

        if (!$tenant) {
            Log::warning('Tentativa de acesso a tenant inexistente ou inativo.', [
                'slug' => $slug,
                'ip' => $request->ip(),
            ]);
            abort(404, 'Prefeitura não encontrada ou inativa.');
        }

        Config::set('database.connections.tenant', array_merge(
            config('database.connections.tenant'),
            [
                'database' => $tenant->database_name,
                'host' => $tenant->database_host ?? config('database.connections.tenant.host'),
            ]
        ));

        DB::purge('tenant');
        DB::reconnect('tenant');

        app()->instance('tenant', $tenant);

        Config::set('cache.prefix', 'tenant_' . $tenant->id);

        return $next($request);
    }

    private function resolveSlug(Request $request): ?string
    {
        $host = $request->getHost();

        if (in_array($host, ['localhost', '127.0.0.1'])) {
            $envSlug = config('app.env') !== 'production' ? env('TENANT_SLUG') : null;

            return $envSlug ?: null;
        }

        $appDomain = config('app.domain', 'vigiacontratos.com.br');
        $domainParts = explode('.', $appDomain);
        $hostParts = explode('.', $host);

        if (count($hostParts) > count($domainParts)) {
            return $hostParts[0];
        }

        return null;
    }
}

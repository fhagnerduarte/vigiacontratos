...
@php
    $title = 'LGPD';
    $subTitle = 'Detalhes da solicitação #' . $solicitacao->id;
@endphp

@section('title', 'LGPD — Solicitação #' . $solicitacao->id)

@section('content')

<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <h6 class="fw-semibold mb-0">Solicitação LGPD #{{ $solicitacao->id }}</h6>
...
<div class="card">
    <div class="card-body p-24">
        <div class="row gy-3">
            <div class="col-md-6">
                <label class="form-label fw-semibold text-neutral-500">Tipo de Solicitação</label>
                <p class="mb-0">
            ...
            <div class="col-md-6">
                <label class="form-label fw-semibold text-neutral-500">Data da Solicitação</label>
                <p class="mb-0">{{ $solicitacao->data_solicitacao?->format('d/m/Y H:i') ?? '-' }}</p>
            </div>

            <div class="col-md-6">
                <label class="form-label fw-semibold text-neutral-500">Data de Execução</label>
...
{{-- Botão Processar: visível apenas para solicitações pendentes não processadas --}}
@if ($solicitacao->status === 'pendente' && !$jaProcessado && auth()->user()->hasPermission('lgpd.processar'))
    <div class="card mt-24">
        <div class="card-header">
            <h6 class="card-title mb-0">Processar Solicitação</h6>
        </div>
        <div class="card-body p-24">
            <form action="{{ route('tenant.lgpd.processar', $solicitacao) }}" method="POST"
                  data-confirm="Tem certeza que deseja marcar esta solicitação como processada? Esta ação não pode ser desfeita."
                  data-confirm-title="Processar solicitação LGPD">
                @csrf
                <div class="mb-16">
                    <label for="observacao" class="form-label fw-semibold">
                        Observação do Processamento <span class="text-danger">*</span>
                    </label>
                    <textarea name="observacao" id="observacao" rows="4"
                              class="form-control @error('observacao') is-invalid @enderror"
                              placeholder="Descreva as ações realizadas para atender esta solicitação (mínimo 10 caracteres)..."
                              required>{{ old('observacao') }}</textarea>
                    @error('observacao')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-neutral-500">Registre o que foi feito para atender a solicitação do titular dos dados.</small>
                </div>
